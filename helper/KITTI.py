...
from helper import flow_transforms
...
def load_flow_from_png(png_path):
    flow = imread(png_path)[:, :, 0:2]
    return (flow.astype(float) - 2**15)/64., flow[:, :, 2]
...
def make_dataset(dir, occ=True, split=0):
    ...
    assert (0 <= split_index <= len(images))
...
def KITTI_occ(root, transform=None, target_transform=None,
    ...
    test_dataset = ListDataset(root, test_list, transform, target_transform, flow_transforms.CenterCrop((320, 1216)),
                               loader=KITTI_loader)
